...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            #2 hidden_error = np.dot(self.weights_hidden_to_output, error)
            ...
            #2 delta_weights_i_h += X[:,None] * hidden_error_term
            
            # Weight step (hidden to output)
            delta_weights_h_o += np.dot(hidden_outputs[:,None], output_error_term[None,:])
            #2 delta_weights_h_o += hidden_outputs[:,None] * output_error_term
